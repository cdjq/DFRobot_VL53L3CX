...
static void writeReg(uint8_t reg, void* pBuf, size_t size) ;
...
static int _ExpandersSetAllIO(void){
    ...
    status = _ExpanderWR(I2cExpAddr0, GPSR, &CurIOVal.bytes[2], 2);
done_err:
    return status;
}
...
static int _ExpanderWR(int I2cExpAddr, int index, uint8_t *data, int n_data) {
    ...
    writeReg(RegAddr[0],RegAddr+1,n_data);
    XNUCLEO53L3A2_PutI2cBus();
    return status;
}
